  ...
  return (
    <section className=''>
      <div className='p-4 hover:bg-green-300'>
        <img
          className='rounded-full h-20 w-33 mr-2 '
          src='/img/logoTecnilibros.gif'
          alt='imagen logo'
          width={250}
          height={100}
          style={{ margin: 'auto' }}
          ...
          <a
            className={
              item.current
                ...
                : 'my-2 px-3 py-2  text-lg text-green-400 font-medium  hover:bg-green-300 hover:text-blue-800'
            }
            key={item.name}
            href={item.href}
          >
  ...
  return (
    <div className='top-2 left-2 absolute mx-auto grid gap-6 peer justify-center bg-orange-400 flex-col border '>
      <div className='flex items-center justify-between px-2 py-2 border-b bg-white'>
        <div>
          <img
            className='h-10 w-10 px-2 py-2 hover:border hover:border-gray-800 hover:border-dashed hover:cursor-pointer'
      ...
      {!open && (
        <div className='h-screen px-3 py-[px] grid gap-y-6 border-b border-black'>
          <Menu />
        </div>
      )}
    </div>
